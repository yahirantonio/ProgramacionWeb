let automovil: {categoria:string, tiempo:number};
let generador;
let pyad:Array<any> = [];
let dyjd:Array<any> = [];
let alum:Array<any> = [];
let public:Array<any> = [];

function getRandomArbitrary(min:number, max:number):number {
  return Math.round(Math.random() * (max - min) + min);
}

const claseConductor=():string =>{
	let condicion:number = getRandomArbitrary(1,6);
...
const generarGrupos= ()=>{
	...
	let tiempo:number = getRandomArbitrary(1000,5000);

	generador = setInterval(()=>{
		let numeroGrupos:number = getRandomArbitrary(1,25);
